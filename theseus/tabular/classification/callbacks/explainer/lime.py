...
class LIMEExplainer(Callbacks):
    ...
    def explain_instance(self, training_data, model, item, feature_names=None, class_names=None):
        '''
    ...
    def on_val_epoch_end(self, logs: Dict=None):
        ...
        exp = self.explain_instance(
            x_train, 
            model, item, 
            feature_names=feature_names, class_names=classnames
        )
        save_path = osp.join(self.save_dir, f'lime_{item_id}.html')
        exp.save_to_file(save_path)
        LOGGER.text(f"LIME figure for a random instance saved at {save_path}", level=LoggerObserver.INFO)
