...
class PartialDependencePlots(Callbacks):
    def __init__(
            self, save_dir, feature_names,  
            target_name, kind="both", num_jobs=1, 
            num_samples=50, figsize=(10, 12), **kwargs
        ) -> None:
    ...
    def on_train_epoch_end(self, logs: Dict=None):
        ...
        LOGGER.text(f"PDP figure saved at {save_path+'.jpg'}", level=LoggerObserver.INFO)
        plt.clf()
