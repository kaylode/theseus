...
class Splitter(Preprocessor):
    def __init__(self, splitter_type='default', save_folder=None, ratio=None, label_column=None, seed=0, n_splits=None, **kwargs):
        super().__init__(**kwargs)

        assert splitter_type in ['default', 'stratified', 'stratifiedkfold'], 'splitter type not supported'
    ...
    def run(self, df):
        ...
        if self.splitter_type == 'default':
            ...
            train_df, val_df = self.splitter(df, stratify=df[[self.label_column]], random_state=self.seed)
            train_df.to_csv(osp.join(self.save_folder, 'train.csv'), index=False)
            val_df.to_csv(osp.join(self.save_folder, 'val.csv'), index=False)
        else:  
            x, y = df.drop(self.label_column, axis=1).values, df[self.label_column].values
            splits = self.splitter.split(x, y)
            for fold_id, (train_ids, val_ids) in enumerate(splits):
                train_df = df.iloc[train_ids]
                val_df = df.iloc[val_ids]
                train_df.to_csv(osp.join(self.save_folder, f'train_fold{fold_id}.csv'), index=False)
                val_df.to_csv(osp.join(self.save_folder, f'val_fold{fold_id}.csv'), index=False)

        self.log(f'Splitted using {self.splitter_type}: {len(train_df)} train, {len(val_df)} validation')
