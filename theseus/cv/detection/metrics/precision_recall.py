...
from .misc import BoxWithLabel, MatchingPairs
...
class DetectionPrecisionRecall(Metric):
    ...
    def value(self):
        total_tp, total_fp, total_fn = self.calculate_cfm(
            self.all_pred_instances, self.all_gt_instances
        )
    ...
    def calculate_pr(self, total_tp, total_fp, total_fn):
        tp_per_class = {i: 0 for i in range(self.num_classes)}

        fp_per_class = {i: 0 for i in range(self.num_classes)}

        fn_per_class = {i: 0 for i in range(self.num_classes)}
