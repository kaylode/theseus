...
class MosaicCollator(BaseCollator):
    ...
    def __call__(self, batch: List[Dict]):
        ...
        for b in range(batch_size):
            ...
            scale_x = self.scale_range[0] + random.random() * (
                self.scale_range[1] - self.scale_range[0]
            )
            scale_y = self.scale_range[0] + random.random() * (
                self.scale_range[1] - self.scale_range[0]
            )
            ...
            candidate_indices = np.random.choice(
                np.setdiff1d(range(batch_size), b), 3, replace=False
            )
            ...
            for i, (img, mask) in enumerate(zip(candidate_images, candidate_masks)):
                if i == 0:  # top-left
                    ...
                    result_image[:, :divid_point_y, divid_point_x:width] = img[
                        :, :divid_point_y, divid_point_x:width
                    ]
                    result_mask[:, :divid_point_y, divid_point_x:width] = mask[
                        :, :divid_point_y, divid_point_x:width
                    ]

                elif i == 2:  # bottom-left
                    result_image[:, divid_point_y:height, :divid_point_x] = img[
                        :, divid_point_y:height, :divid_point_x
                    ]
                    result_mask[:, divid_point_y:height, :divid_point_x] = mask[
                        :, divid_point_y:height, :divid_point_x
                    ]

                else:  # bottom-right
                    result_image[:, divid_point_y:height, divid_point_x:width] = img[
                        :, divid_point_y:height, divid_point_x:width
                    ]
                    result_mask[:, divid_point_y:height, divid_point_x:width] = mask[
                        :, divid_point_y:height, divid_point_x:width
                    ]
